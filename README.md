...
<p align="center">
  <a href="https://habitus.choiszt.com"><img src="https://img.shields.io/badge/🌐_Product_Page-habitus.choiszt.com-38bdf8?style=for-the-badge" alt="Product Page"></a>
  <img src="https://img.shields.io/badge/macOS_14+-required-1e3a5f?style=for-the-badge" alt="macOS 14+">
  <img src="https://img.shields.io/badge/version-v0.1.0_beta-505878?style=for-the-badge" alt="v0.1.0">
</p>
...
## `.habitus` — Three-Channel Memory

All behavioral data lives in `~/.habitus/` on your machine. The memory system uses a three-channel architecture inspired by how human memory works:

```
~/.habitus/
├── config.yaml                     # Your preferences (auto-generated)
├── habitus.db                      # SQLite: profiles, history, sessions
└── sessions/
    └── {profile_id}/
        └── {session_id}/
            ├── events.json         # Raw file operation trace (JSONL)
            └── media/              # Optional screenshots
```

### Procedural Channel
Statistical features extracted from your file traces — read/write ratios, edit sizes, nesting depth, timing patterns. This is the **what** of your behavior, encoded as numerical fingerprints that persist across sessions.

### Semantic Channel
Natural language descriptions of your work patterns, generated by distilling file deltas through LLM encoding. This is the **why** — not just that you edited 5 files, but that you were *"refactoring a classifier with incremental test-driven changes."*

### Episodic Channel
Cross-session consistency tracking and behavioral drift detection. This is the **when** — which patterns are stable habits vs. one-off anomalies. Habitus segments your work into episodes and tracks how your behavior evolves over time.

The three channels consolidate into a single behavioral profile with confidence scores. The more you work, the more accurate it gets.
...
- All data stays on your machine — `~/.habitus/` is yours alone
- No cloud. No telemetry. No account required
- No file contents are read — only metadata, structure, and delta patterns
- Behavioral profiles never leave your laptop unless you explicitly export them
...
**Habitus is free.** Currently in private beta.

No subscription. No account. No trial period.

[Visit Product Page →](https://habitus.choiszt.com)
